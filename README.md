...
- `v2.0.24` — bridge-script iframe content extraction (Phase A2 /
  ...
  JS); `scripts/sync-bridge-script.js` regenerates the wrapper at
  pre-commit time. Closes AUDIT-A item #15 / ARCH-001. Gate-A 315/8/0
  preserved.
...
- `v2.0.16` — A11Y-001 closed: color-contrast (`--shell-text-muted`
  ...
  to sibling of `<summary>`). Gate-a11y 27/0/0 with no masked.
  Phase 3 of 9 polish phases.
- `v2.0.15` — SEC-006 closed: 3 slide-keyed dicts on `state` migrated
  to `Object.create(null)`, registry write site + iframe lookup
  reject reserved slide IDs (`__proto__`, `constructor`, `prototype`).
  Fixture + 8 new specs. Phase 2 of 9 polish phases.
- `v2.0.14` — SEC-004 closed: inbound bridge messages now
  schema-validated (every type except hello which keeps its bespoke
  protocol-mismatch UX). Object.create(null) flatten blocks
  prototype-keyed payload smuggling. 9 new specs. Phase 1 of 9
  polish phases.
...
- `v2.0.11` — Inspector empty-state guidance card (persistent
  "how do I edit what's already here?" answer when no selection)
- `v2.0.10` — W/H in basic mode (geometry section no longer
  advanced-only; display/position/z/X/Y still advanced)
